...
CATEGORIES_FILE = WORKING_DATA / "column_categories.json"
CLEANING_FILE = WORKING_DATA / "cleaning_actions.json"
...
def create_scatter_plot(numeric_series, issues):
    ...
    for col_name in df.columns:
        ...
        with action_col:
            ...
            if len(issues['outliers']) > 0:
                ...
                # Slider first to get current value
                threshold = st.slider(
                    f"Threshold",
                    1.0, 5.0, col_actions['outlier_threshold'],
                    key=f"{col_name}_thresh",
                    step=0.1
                )
                
                # Calculate outlier count at CURRENT slider position (for live display)
        ...
        with plot_col:
            # Generate preview with current actions
            ...
            # Use ORIGINAL issues for coloring (don't recalculate after actions)
            preview_issues = {
                'parsing_errors': [],
                'outliers': issues['outliers'],  # Use original outliers
                'negatives': issues['negatives'],  # Use original negatives
                'missing': []
            }
            
            # Mark filled parse errors as purple
            if col_actions['parsing_errors'] in ['mean', 'median']:
                preview_issues['parsing_errors'] = issues['parsing_errors']
            
            # Mark filled missing values as cyan
